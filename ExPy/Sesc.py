...
def criar_janela_secundaria():
    ...
    def capturaraudio(entry):
        nomefalar=pyttsx3.init()
        nomefalar.say('Diga Seu nome')
        nomefalar.runAndWait()
        with sr.Microphone() as source:
            rec.adjust_for_ambient_noise(source)
            audio = rec.listen(source)
            try:
                transnome = rec.recognize_google(audio, language='pt-BR')
                entry.insert(tk.END, transnome)  # Inserir o texto reconhecido na entrada
            except sr.UnknownValueError:
                print("Não foi possível reconhecer o áudio.")
            except sr.RequestError as e:
                print(f"Erro ao chamar o serviço de reconhecimento de fala; {e}")
    def iniciar_reconhecimento(entry1):
    ...
    janela_secundaria.after(100, lambda: iniciar_reconhecimento(nomeentrada))
